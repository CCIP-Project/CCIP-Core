...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CCIP);
    ...
    if (!settings.contains("nAnonymizeCcipAmount"))
        settings.setValue("nAnonymizeCcipAmount", 1000);

    nAnonymizeCcipAmount = settings.value("nAnonymizeCcipAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCcipAmount"))
        SoftSetArg("-anonymizeccipamount", settings.value("nAnonymizeCcipAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ccip.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCcipAmount:
            return QVariant(nAnonymizeCcipAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCcipAmount:
            nAnonymizeCcipAmount = value.toInt();
            settings.setValue("nAnonymizeCcipAmount", nAnonymizeCcipAmount);
            emit anonymizeCcipAmountChanged(nAnonymizeCcipAmount);
            break;
        case CoinControlFeatures:
