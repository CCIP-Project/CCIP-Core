...
std::set<std::string> ZCcipControlDialog::setSelectedMints;
std::set<CMintMeta> ZCcipControlDialog::setMints;

ZCcipControlDialog::ZCcipControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCcipControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCcipControlDialog::~ZCcipControlDialog()
{
    delete ui;
}

void ZCcipControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCcipControlDialog::updateList()
{
...
void ZCcipControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCcipControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCcip_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCcipControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCcipControlDialog::GetSelectedMints()
{
...
void ZCcipControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
