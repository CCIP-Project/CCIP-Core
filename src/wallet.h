...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CCIP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCCIP send
enum ZerocoinSpendStatus {
    ZCCIP_SPEND_OKAY = 0,                            // No error
    ZCCIP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCCIP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCCIP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCCIP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCCIP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCCIP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCCIP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCCIP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCCIP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCCIP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCCIP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCCIP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCCIP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCCIP_SPENT_USED_ZCCIP = 14,                      // Coin has already been spend
    ZCCIP_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCCIP_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZCcipBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZCCIPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzCCIPWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCCIPTracker> zpivTracker;
    ...
    void setZWallet(CzCCIPWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzCCIPTracker>(new CzCCIPTracker(strWalletFile));
    }

    CzCCIPWallet* getZWallet() { return zwalletMain; }
    ...
    void setZCcipAutoBackups(bool fEnabled)
    {
    ...
    /** zCCIP reset */
    boost::signals2::signal<void()> NotifyzCCIPReset;
