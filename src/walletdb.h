...
public:
    ...
    bool WriteZCCIPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCCIPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCCIPSeed_deprecated(uint256& seed);
    bool EraseZCCIPSeed();
    bool EraseZCCIPSeed_deprecated();

    bool WriteZCCIPCount(const uint32_t& nCount);
    bool ReadZCCIPCount(uint32_t& nCount);
