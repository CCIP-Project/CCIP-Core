// Copyright (c) 2018 The CCIP developers
...
CzCCIPTracker::CzCCIPTracker(std::string strWalletFile)
{
...
CzCCIPTracker::~CzCCIPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCCIPTracker::Init()
{
...
bool CzCCIPTracker::Archive(CMintMeta& meta)
{
...
bool CzCCIPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCCIPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCCIPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCCIPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCCIPTracker::GetSerialHashes()
{
...
CAmount CzCCIPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCCIPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCCIPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCCIPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCCIPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCCIPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCCIPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCCIPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCCIPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCCIPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCCIPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCCIPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCCIPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCCIPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCCIPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCCIPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCCIPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
