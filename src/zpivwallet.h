...
#ifndef CCIP_ZCCIPWALLET_H
#define CCIP_ZCCIPWALLET_H
...
public:
    CzCCIPWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCCIP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCCIP(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //CCIP_ZCCIPWALLET_H
